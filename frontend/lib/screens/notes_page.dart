...
class _NotesPageState extends State<NotesPage> {
  ...
  Future<void> _load() async {
    ...
    try { _notes = await _service.listNotes(noteType: 'note'); } catch (_) {}
  ...
  Future<void> _add() async {
    ...
    final result = await showDialog<Map<String, dynamic>>(context: context, builder: (ctx) => AlertDialog(
      ...
      title: Text('新建笔记', style: TextStyle(color: AppColors.text(brightness))),
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        TextField(controller: titleCtrl, style: TextStyle(color: AppColors.text(brightness)), decoration: InputDecoration(labelText: '标题', labelStyle: TextStyle(color: AppColors.textSecondary(brightness)))),
        const SizedBox(height: 8),
        TextField(controller: contentCtrl, maxLines: 3, style: TextStyle(color: AppColors.text(brightness)), decoration: InputDecoration(labelText: '内容', labelStyle: TextStyle(color: AppColors.textSecondary(brightness)))),
      ]),
      actions: [
        TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('取消')),
        TextButton(onPressed: () => Navigator.pop(ctx, {'title': titleCtrl.text, 'content': contentCtrl.text, 'note_type': 'note'}), child: const Text('保存', style: TextStyle(color: AppColors.accent))),
      ],
    ));
    if (result != null && (result['title'] as String).isNotEmpty) { await _service.createNote(result); _load(); }
  }

  Future<void> _delete(String id) async { await _service.deleteNote(id); _load(); }

  void _showDetail(Map<String, dynamic> note, Brightness b) {
      ...
      body: _loading ? const Center(child: CircularProgressIndicator(color: AppColors.accent))
          ...
          : RefreshIndicator(color: AppColors.accent, backgroundColor: AppColors.card(brightness), onRefresh: _load,
              child: ListView.builder(padding: const EdgeInsets.fromLTRB(16, 8, 16, 80), itemCount: _notes.length, itemBuilder: (_, i) {
                final n = _notes[i];
                return Dismissible(key: ValueKey(n['id']), direction: DismissDirection.endToStart, onDismissed: (_) => _delete(n['id'] as String),
                  child: GestureDetector(
                    onTap: () => _showDetail(n, brightness),
                    child: Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: AppColors.card(brightness), borderRadius: BorderRadius.circular(12)),
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text(n['title'] as String? ?? '', style: TextStyle(color: AppColors.text(brightness), fontSize: 14, fontWeight: FontWeight.w500)),
                        if ((n['content'] as String? ?? '').isNotEmpty) ...[const SizedBox(height: 4), Text(n['content'] as String, maxLines: 3, overflow: TextOverflow.ellipsis, style: TextStyle(color: AppColors.textSecondary(brightness), fontSize: 12))],
                      ]),
                    ),
                  ),
                );
